...
const formatDateTime = (val) => {
  if (!val) return { date: 'Today', time: '10:00 AM', full: 'Today' };
  try {
    const d = new Date(val);
    if (isNaN(d.getTime())) return { date: 'Today', time: '10:00 AM', full: 'Today' };
    return {
    ...
    return { date: 'Today', time: '10:00 AM', full: 'Today' };
  }
};
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                  <div style={{ fontWeight: 600, fontSize: '14px', color: '#0f172a' }}>
                    {appt.visitorId?.fullName || appt.visitor?.fullName || 'Visitor'}
                  </div>
                  <StatusPill status={appt.status} />
                </div>
                <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '4px' }}>
                  {appt.visitorId?.company || appt.visitor?.company || 'Independent'} • {appt.visitorId?.email || appt.visitor?.email || ''}
                </div>
                ...
                <div style={{ fontSize: '11px', color: '#64748b', marginBottom: '12px' }}>
                  Arrival: {formatDateTime(appt.scheduledStartTime || appt.scheduledDate).full}
                </div>
                  ...
                  <tr key={appt._id}>
                    <td>
                      ...
                      <div style={{ fontSize: '11px', color: '#64748b' }}>
                        {appt.visitorId?.email || appt.visitor?.email || ''}
                      </div>
                    </td>
                    <td>{appt.visitorId?.company || appt.visitor?.company || 'Independent'}</td>
                    <td>
                    ...
                    <td>
                      <div>{formatDateTime(appt.scheduledStartTime || appt.scheduledDate).date}</div>
                      <div style={{ fontSize: '11px', color: '#64748b' }}>
                        {formatDateTime(appt.scheduledStartTime || appt.scheduledDate).time}
                      </div>
                    </td>
                    <td>
                      <StatusPill status={appt.status} />
        ...
        {selectedAppt && (
          <div>
            <p style={{ marginBottom: '14px', fontSize: '13px', color: '#475569' }}>
              Confirm authorization for <strong>{selectedAppt.visitorId?.fullName || selectedAppt.visitor?.fullName || 'Visitor'}</strong> ({selectedAppt.visitorId?.company || selectedAppt.visitor?.company || 'Independent'}). An official QR pass badge will be generated in the database.
            </p>
        ...
        >
          <div style={{ textAlign: 'center', padding: '10px 0' }}>
            <PassBadge
              ...
              visitor={selectedBadgePass.visitorId || selectedBadgePass.visitor}
              host={selectedBadgePass.hostId || selectedBadgePass.host}
              organization={selectedBadgePass.organizationId || selectedBadgePass.organization}
            />
          </div>
        </Modal>
      )}
