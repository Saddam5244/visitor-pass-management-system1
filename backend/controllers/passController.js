  ...
  try {
    ...
    if (qrData.startsWith('{')) {
      try {
        ...
        passNumber = parsed.passNumber || parsed.qrToken || parsed.passId || passNumber;
      } catch (e) {
        // Not JSON, continue with raw string
      }
    }

    // If QR code is a URL, extract pass number or ID from path
    if (passNumber.includes('/pass/')) {
      const parts = passNumber.split('/pass/')[1].split('?')[0].split('/');
      if (parts[0]) passNumber = parts[0];
    }
    ...
    if (mongoose.Types.ObjectId.isValid(passNumber)) {
      queryConditions.push({ _id: passNumber });
    }

    const pass = await Pass.findOne({ $or: queryConditions })
      .populate('visitorId')
      .populate('hostId', 'name email department phone organizationName')
      .populate('appointmentId')
      .populate('organizationId');
