...
const scanCheckIn = async (req, res) => {
  try {
    ...
    if (!pass && (req.body.isGuest || req.body.createGuestIfMissing)) {
      const defaultHost = (await User.findOne({ role: 'employee' })) || req.user;
      let visitor = await Visitor.findOne({ phone: '9999999999' });
      if (!visitor) {
        visitor = await Visitor.create({
          fullName: req.body.guestName || 'Guest Visitor',
          email: `guest-${Date.now()}@visitorpass.com`,
          phone: '9999999999',
          company: req.body.guestCompany || 'External Visitor',
          isOtpVerified: true,
        });
      }
      const newPassNum = `VP-GUEST-${Math.floor(1000 + Math.random() * 9000)}`;
      pass = await Pass.create({
        passNumber: newPassNum,
        visitorId: visitor._id,
        hostId: defaultHost ? defaultHost._id : req.user?._id,
        badgeType: 'GUEST',
        qrToken: `GUEST-${Date.now()}-${Math.floor(Math.random() * 10000)}`,
        qrCodeData: passNumber.trim(),
        validFrom: new Date(),
        validTo: new Date(Date.now() + 8 * 3600000),
        status: 'ISSUED',
        purpose: req.body.purpose || 'External Visitor Entry',
      });
      pass.visitorId = visitor;
      pass.hostId = defaultHost;
    }
