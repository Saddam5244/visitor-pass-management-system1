...
const getAppointments = async (req, res) => {
  try {
    ...
    // Role scoping: Employee sees appointments where they are the host, or demo/general requests
    if (req.user.role === 'employee') {
      const demoHost = await User.findOne({ email: 'host@visitorpass.com' });
      const hostIds = [req.user._id];
      if (demoHost) hostIds.push(demoHost._id);
      query.$or = [{ hostId: { $in: hostIds } }, { hostId: null }];
    }
...
const approveAppointment = async (req, res) => {
  try {
    ...
    // Role authorization: Employee can approve their own appointments or demo/general requests
    if (req.user.role === 'employee' && appointment.hostId && appointment.hostId._id.toString() !== req.user._id.toString()) {
      if (appointment.hostId.email === 'host@visitorpass.com' || !appointment.hostId) {
        appointment.hostId = req.user._id;
      } else {
